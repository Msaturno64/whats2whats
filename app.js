...
const extractWaidFromContactMessage = (contactMessage) => {
    const vcard = contactMessage.vcard;
    const waidMatch = vcard.match(/waid=(\d+)/);
    return waidMatch ? waidMatch[1] : null;
};
...
const main = async () => {
    ...
    const sock1 = await initWhatsApp('auth_info1', 4000, async (m) => {
        ...
        // Si el mensaje es enviado por whatsapp2 y contiene solo un número, crear grupo
        if (textMessage && !sender.endsWith('@g.us') && textMessage.match(/^\d+$/)) {
            const clientNumber = `${textMessage}@s.whatsapp.net`;
            if (!groupIds[clientNumber]) {
                const participants = ['']; // WhatsApp2
                const groupName = textMessage; // Usar el número del cliente como nombre del grupo
                groupIds[clientNumber] = await createGroup(sock1, groupName, participants);
                saveGroupIds(); // Guardar cambios después de crear el grupo
                console.log(`Group ID stored for ${clientNumber}: ${groupIds[clientNumber]}`);
            }
            return;
        }

        // Si el mensaje es un contacto enviado por whatsapp2
        if (msg.message.contactMessage && !sender.endsWith('@g.us')) {
            const clientNumber = extractWaidFromContactMessage(msg.message.contactMessage);
            if (clientNumber) {
                const clientJid = `${clientNumber}@s.whatsapp.net`;
                if (!groupIds[clientJid]) {
                    const participants = ['']; // WhatsApp2
                    const groupName = clientNumber; // Usar el número del cliente como nombre del grupo
                    groupIds[clientJid] = await createGroup(sock1, groupName, participants);
                    saveGroupIds(); // Guardar cambios después de crear el grupo
                    console.log(`Group ID stored for ${clientJid}: ${groupIds[clientJid]}`);
                }
            }
            return;
        }
